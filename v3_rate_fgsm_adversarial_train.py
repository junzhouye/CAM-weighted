...
for epoch in range(1, args.epochs + 1):
    ...
    rate_weight_fgsm_adversarial_train_epoch(model, device, train_loader, optimizer, epoch, epsilon, alpha,
                                             rate)
    ...
    if (epoch % args.save_freq == 0) or (epoch == args.epochs):
        torch.save(model.state_dict(),
                   os.path.join(model_dir, 'v3_rate_fgsm_rate{}_epoch{}.pth'.format(rate, epoch)))
