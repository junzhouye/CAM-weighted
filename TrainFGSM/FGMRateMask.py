...
class FGMRateTrain(BaseTrain):
    def getMask(self, data, target, fgsm_perturbed, mode):
        ...
        rate_v = sorted_grad.values[int(len_sort_grad * (1 - mode))]
        mask = (torch.abs(grad.clone()) > (torch.zeros_like(grad) + rate_v))
